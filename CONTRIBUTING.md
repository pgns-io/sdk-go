...
- `*_test.go` — additional test coverage

## What to open an issue for

Most source files (`*.go`, `go.mod`, etc.) are generated from the upstream monorepo. PRs that modify these files can't be merged directly because the next sync would overwrite them.
...
```bash
git clone https://github.com/pgns-io/sdk-go.git
cd sdk-go
```
